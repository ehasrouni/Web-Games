...
Within this web application, there's 2 games:
A car racing game (infographic application designed with WebGL).
A crossword puzzle game with multiple difficulty levels (with Angular). The game can be played in solo mode or online.
...
2- Run npm-start or ng-serve in the client and server repositories, your browser should open automatically afterwards. If it did not, go to http://localhost:4200/ to launch the app.
...
Execute `ng test --watch=false --code-coverage` to get a test report with more information [Karma](https://karma-runner.github.io) on the client.
